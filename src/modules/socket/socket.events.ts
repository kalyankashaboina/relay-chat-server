...
export function registerSocketEvents(io: Server) {
  io.on("connection", async (socket: AuthenticatedSocket) => {
    ...
    socket.on(
      "message:send",
      async ({ conversationId, content, tempId }: SendMessagePayload) => {
    ...
    socket.on(
      "message:delete",
      async ({ messageId }: DeleteMessagePayload) => {
    ...
    socket.on(
      "conversation:read",
      async ({ conversationId }: ReadConversationPayload) => {
        await assertConversationMember(conversationId, userId);
    ...
    socket.on(
      "typing:start",
      async ({ conversationId }: TypingPayload) => {
    ...
    socket.on(
      "call:initiate",
      ({ toUserId, type }: CallInitiatePayload) => {
